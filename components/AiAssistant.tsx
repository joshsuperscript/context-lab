...
interface AiAssistantProps {
  ...
  embedded: boolean  // true = renders inline, false = floating button
}

export default function AiAssistant({ fileId, section, onInsert, embedded }: AiAssistantProps) {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      if ((e as Error).name !== 'AbortError') toast.error('Request failed')
    } finally {
      setStreaming(false)
    }
  }
    ...
    <div className="flex flex-col h-full">
        ...
        {!started && (
          <div className="text-center py-6 space-y-3">
            <p className="text-xs text-gray-500 leading-relaxed">
              I can help you fill in this document through structured questions, or answer questions and propose edits based on existing published content.
            </p>
            <div className="flex flex-col gap-2">
              <button
                onClick={() => send('Start the interview — ask me your first question about this document.')}
                className="w-full text-xs px-3 py-2 bg-[#00A3FF] text-white rounded-lg hover:bg-[#0091e6]"
              >
                Start interview
              </button>
              <button
                onClick={() => send('What existing docs in this section should I know about before writing?')}
                className="w-full text-xs px-3 py-2 border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50"
              >
                Show me related docs
              </button>
            </div>
          </div>
        )}
          ...
          return (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[90%] text-xs rounded-xl px-3 py-2 ${
                msg.role === 'user' ? 'bg-[#00A3FF] text-white' : 'bg-gray-100 text-gray-800'
              }`}>
                {displayText && <p className="whitespace-pre-wrap">{displayText}</p>}
                {insertText && (
                  <div className="mt-2 border-t border-gray-200 pt-2">
                    <div className="bg-white rounded-lg p-2 font-mono text-xs text-gray-700 whitespace-pre-wrap border border-gray-200">
                      {insertText}
                    </div>
                    <button
                      onClick={() => { onInsert(insertText); toast.success('Inserted') }}
      ...
      <div className="p-3 border-t border-gray-200 flex gap-2">
        <input
          ...
          placeholder={started ? 'Ask or answer…' : 'Or type a question…'}
          disabled={streaming}
        ...
        <button
          ...
          className="p-2 bg-[#00A3FF] text-white rounded-lg disabled:opacity-40"
        >
          <Send size={13} />
        </button>
      </div>
  ...
  return (
    <div className="fixed bottom-6 right-6 w-80 bg-white rounded-2xl shadow-xl border border-gray-200 flex flex-col overflow-hidden" style={{ height: 480 }}>
      <div className="flex items-center justify-between px-4 py-3 bg-[#00A3FF] text-white shrink-0">
